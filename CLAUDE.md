...
- **В процессе (начато 2026-07-20): перенос хардкод-текстов в БД (подготовка
  ...
  **ничего не меняет для пользователей**, пока не построено переключение
  ...
  2. **Дедуп общих фраз-сообщений** (не кнопок) в `BotText` — одна запись на
     уникальный текст, а не одна на место использования. **В процессе.**
     Приятная находка: в БД уже есть готовый набор строк `err_*` (созданы
     2026-07-06, ДО начала этого проекта, видимо владельцем через
     category_admin — с готовым `text_en`), но нигде не подключены в коде:
     `err_not_owner`, `err_listing_404`, `err_no_rights`, `err_invalid_data`,
     `err_invalid_id`, `err_field_404`, `err_field_type`, `err_save_retry`.
     Перед добавлением новой строки в BotText — сначала проверять, нет ли
     уже готовой под этот текст (`sqlite3 dev.db "SELECT code, text_ru FROM
     BotText WHERE code LIKE 'err_%'"`).
     **Готово:** «Некорректные данные.» (23 места, 6 файлов) →
     `err_invalid_data`; «Объявление не найдено.» (16 мест, 7 файлов,
     включая `events_add.py` — там тоже про listing, не про мероприятие)
     → `err_listing_404`. Паттерн подключения (варьируется вызовом-
     обёрткой, но суть одна):
     ```python
     await cb.answer(await get_text("err_listing_404", "ru") or "Объявление не найдено.", show_alert=True)
     ```
     (`get_text` берётся либо из `app.routers.utils`, либо из `app.texts`
     — сигнатуры совместимы, в `market_view.py` уже был импортирован
     второй вариант, не меняли).
     «Можно редактировать только свои объявления.» (17 мест, 3 файла:
     `market_edit.py`/`market_edit_photos.py`/`market_edit_overview.py`
     — не было в исходном списке плана, но топ-3 по частоте) →
     `err_not_owner`.
     **Осталось из исходного списка кандидатов:**
     «Можно редактировать только свои услуги.» (18, готового кода в БД
     нет — завести новый), «Некорректная ссылка.» (12), «Фото не найдено.»
