...
- **Инцидент с Antigravity (2026-07-22):** владелец параллельно запускал
  Antigravity (агент из другого приложения) на аудит `category_admin.py` —
  тот вместо аудита стал вносить правки и сломал часть функционала
  (пустой экран категорий). Откачено `git checkout`/`git stash` к последнему
  коммиту `3a82385`; Antigravity остановлен владельцем, использовать его для
  правок в этом проекте больше не планируется (только для стороннего аудита
  по отдельному запросу). Правки Antigravity лежат в
  `git stash@{0}` (`antigravity-mess-2026-07-22`), не применены.
- **Превью/таблица + сортировка + поиск теперь и у Релизов/Исполнителей
  (2026-07-22).** По образцу уже существовавшего у «📦 Объявления» (`cat*`
  в JS) паттерна — настраиваемые столбцы (показать/скрыть, drag-переупорядо-
  чивание, resize), сортировка по клику на заголовок, режим превью/таблица,
  пагинация — заведены свои копии для Релизов (`rel*`) и Исполнителей
  (`art*`), т.к. это плоские списки без дерева категорий (в отличие от
  Объявлений, где тот же паттерн завязан на `catId`/`stype` текущей
  категории). `/api/releases` и `/api/artists` соответственно получили
  `offset`/`limit`/`sort`/`order`/`q`/`filters`.
  **Общий + постолбцовый поиск** — во всех трёх (Объявления/Релизы/
  Исполнители): один общий инпут (`q`, ищет по нескольким полям сразу) —
  живёт статично СНАРУЖИ перерисовываемого блока (как `#user-search`),
  чтобы не терять фокус при вводе; плюс инпут под каждым фильтруемым
  заголовком столбца (`filters` — JSON `{колонка: подстрока}`, LIKE по
  соответствующей SQL-колонке). Столбцовые инпуты живут ВНУТРИ
  перерисовываемой таблицы (в `<thead>`, часть текущего набора столбцов) —
  для них заведён общий хелпер `withFocusPreserved()` (сохраняет id и
  позицию курсора активного инпута перед перерисовкой, восстанавливает
  после), иначе каждый символ ввода сбрасывал бы фокус.
  **Клик по счётчику «Открытий»/«Просмотры» → кто и сколько раз открывал.**
  Новые эндпоинты `/api/listing/{id}/openers` (группировка `listing_views`
  по `user_id`, работает и для Барахолки/Услуг/Вакансий, и для Релизов —
  у них общая таблица `listing`) и `/api/artist/{id}/openers` (группировка
  `analytics_events` по `user_id`, `event_type='artist_opened'`). Общая
  модалка `openOpenersModal(kind, id, title)` на все три раздела. В
  таблице объявлений счётчик — внутри строки с `onclick` на всю строку
  (открывает карточку) — у ссылки счётчика обязателен
  `event.stopPropagation()`, иначе клик заодно открывал бы карточку.
  `listing_views.created_at` хранится как unix-эпоха (INTEGER), в отличие
  от `analytics_events.created_at` (уже строка) — отформатирован на
  бэкенде через `datetime(..., 'unixepoch', 'localtime')`, чтобы отдавать
  фронтенду единый строковый формат под общую `fmtDateTime()`.
  **Найденный и исправленный баг (тот же день, по живой проверке
  владельцем):** при нулевом результате постолбцового фильтра в табличном
  режиме весь `<table>` (включая строку с полями фильтров) заменялся на
  простое «не найдено» — вместе с таблицей исчезало и то самое поле, где
  пользователь набирал текст, поправить фильтр было нечем кроме
  перезагрузки страницы. Исправлено в `loadReleases`/`loadArtists`/
  `catalogShowListings`: при 0 строк и активном табличном режиме таблица
  (шапка + строка фильтров) всё равно рендерится (пустым телом), сообщение
  «не найдено» добавляется под ней, а не вместо неё. В режиме превью
  (карточки) эта проблема не возникает — там нет столбцовых фильтров.
- **Тот же паттерн (превью/таблица + сортировка + столбцы + поиск)
  распространён и на «🔥 Топ карточек» в Аналитике (2026-07-22, по
  просьбе владельца).** Раньше это был жёсткий `LIMIT 20` без пагинации,
  сортировки и поиска — теперь `/api/analytics/top_cards` тоже отдаёт
  `total`/`offset`/`limit`/`rows` и принимает `sort`/`order`/`q`/`filters`,
  фронтенд получил `tc*`-функции (по образцу `rel*`/`art*`). Поисковая
  строка живёт в отдельном тулбаре `#an-topcards-toolbar` (статично, вне
  `#an-content`, который целиком перерисовывается при переключении под-вкладок
  аналитики) — показывается/прячется в `anSwitch()` по текущей секции.
  Владелец специально уточнил: 2-колоночная сетка превью не приоритет,
  важнее «оперативный доступ ко всем параметрам» — поэтому у Топ карточек
  (как и у Релизов/Исполнителей) режим «Таблица» выбран умолчанием
  (`localStorage` ключ `admin_tc_view_mode`), а не «Превью». Счётчик
  «Открытий» в таблице тоже кликабелен → та же модалка `openOpenersModal`,
  что и у остальных разделов (`listing_id` у топ-карточки — это и есть
  `listing.id`, тот же эндпоинт `/api/listing/{id}/openers` подходит).
  **Найденный и исправленный баг регистронезависимости (тот же день):**
  поиск «микрофон» (нижний регистр) не находил «Микрофон Shure A87» —
  классическая ловушка SQLite `LIKE`/`LOWER()`, которые сворачивают регистр
  только для ASCII, кириллица не сворачивается (та же природа бага, что и
  раньше в поиске пользователей веб-админки — см. запись выше про
  переход на `casefold()` в `/api/users`). Из-за этого при добавлении
  поиска/фильтров в Релизы,
  Исполнителей, Объявления и Топ карточек в этой же сессии баг был
  тиражирован сразу в четыре места. Исправлено везде разом: общий бэкенд-
  хелпер `_casefold_filter_paginate(rows, q, q_fields, f_dict,
  filter_fields, offset, limit)` — SQL теперь отдаёт только структурные
  условия (`category_id`, `section`, `activity`, `status != 'deleted'` и
  т.п. — не текстовые, регистр не важен) и полную отсортированную выборку
  БЕЗ `LIMIT/OFFSET`; текстовый поиск/фильтры и постраничная нарезка —
  на Python через `str.casefold()`. Таблицы в проекте небольшие (десятки–
  сотни строк, community-бот), полная выборка на каждый запрос не проблема
  — тот же осознанный компромисс, что и раньше для поиска пользователей.
  **Урок:** при копировании готового паттерна поиска/фильтра на новый
  раздел — сразу проверять кириллицу в нижнем регистре, а не только
  ASCII/точное совпадение регистра, иначе баг тиражируется вместе с
  паттерном.
  **Пагинация с номерами страниц (та же сессия, по просьбе владельца):**
  раньше только «◀ N/M ▶» — при сотнях строк дойти до дальней страницы
  можно было только кликая много раз подряд. Общая функция
  `paginationHtml(total, limit, offset, gotoFnName)` — кнопки «« Первая» /
  номера страниц (с «…» для дальних) / «Последняя »» / поле «№ стр.» с
  переходом по Enter; используется в Релизах/Исполнителях/Объявлениях/
  Топ карточках (`gotoFnName` — имя JS-функции вида `relGoPage`/`artGoPage`/
  `catGoPage`/`tcGoPage`, т.к. `catalogShowListings` требует ещё и
  `catId`/`stype`, которых нет в замыкании — `catGoPage` берёт их из уже
  существующих `_catCurCatId`/`_catCurStype`).
  **Докатано на весь остальной проект (та же сессия, по просьбе владельца
  «везде, где есть списки таблицами»):** «✉️ Обратная связь» (`fbPage`),
  «📝 Тексты» (`txtPage` — старая строка с выбором «на странице 10/20/50/100»
  осталась отдельной строкой над `paginationHtml()`, не смешаны), «👤 Авторы»
  в Аналитике (`anOwnersGoPage` — обёртка, т.к. исходная `anOwners(el, offset)`
  принимает `el` первым аргументом, которого нет в `gotoFnName(offset)`),
  и модалка «drill» по клику на цифры в дереве категорий Аналитики —
  список объявлений (`drillListingsGoPage`) и список открытий/контактов
  (`drillViewsGoPage`), тоже обёртки поверх `_drillState` по той же причине
  (исходные функции принимают `catId/stype/title/action`, которых нет в
  замыкании `gotoFnName`). **Сознательно не трогал:** «👤 Пользователи»
  (там вообще нет пагинации — фиксированный `limit=100/1000` без контролов,
  это отдельная задача, не апгрейд существующей пагинации) и
  `openDrillUsers` (список пользователей по клику на «польз.» в дереве —
  тоже без пагинации, отдаёт сразу всех).
...
- **Повторный аудит остальных разделов (Барахолка/Вакансии/Афиша/Релизы,
  2026-07-21-22) — большая часть закрыта через Antigravity.** По итогам
  7 параллельных агентов, прошедших ~18 000 строк, найдено и передано на
  исправление (`ANTIGRAVITY_TASK.md`, файл вне git — разовая инструкция):
  - **Security/данные — закрыто:** `feedback.py` — перехват чужого
    обращения через `fb:need:<id>`/`fb:noneed:<id>` (нет было проверки
    владения) и гонка в общей цели «быстрого ответа» админа без TTL —
    оба исправлены (проверка `rowcount`/владения + TTL 5 минут на
    `_admin_reply_target`). `events_add.py` — обновление
    `events_meta.city_text` без проверки владельца и отсутствие проверки
    `rowcount` в 10 apply-хендлерах после публикации события — тоже
    закрыто (та же `EXISTS(...)`-обвязка, что уже была в price-хендлерах
    этого файла).
  - **Мёртвый код — закрыто:** весь файл `market_edit.py` (369 строк,
    старый флоу `edit_listing:`, не генерировался нигде — аналог
    `services_edit.py` из прошлой сессии) удалён; 5 мёртвых
    callback-хендлеров в `market_view.py`; дублирующий класс `EditListing`
    в `app/states.py` (было определено дважды) + неиспользуемый
    `AddListing`; мёртвый хендлер `edit_vacancy_overview:` в
    `vacancy_edit.py` (тест на него переписан на реальный
    `vacancy_edit_overview:`); мёртвая параллельная реализация корневого
    экрана Афиши в `events_view.py` — **перед удалением коды BotText
    (`btn_search`/`btn_my_events`/`btn_near_events`/`btn_add_event`)
    перенесены в реально используемую `events_main_inline()`
    (`app/keyboards.py`)**, которая раньше их хардкодила.
  - **Крупный побочный результат: доп.поля сразу после публикации
    заработали и для Вакансий.** При этом весь старый мёртвый блок
    `vacancy_add.py` (доп.поля мастера → экран подтверждения →
    `vac_publish`, ~350 строк, был недостижим — реальная публикация
    сохраняла вакансию сразу на шаге цены) не просто удалён, а **заменён**
    на тот же паттерн, что уже работал в `market_add.py`/`services_add.py`
    (`start_extra_fields_for_category` сразу после публикации, если у
    категории есть поля) — решение по стоп-пункту из `ANTIGRAVITY_TASK.md`
    фактически принято в пользу третьего варианта (не «удалить как есть»
    и не «реанимировать старую ветку», а унифицировать с остальными
    разделами). `user_extra_fields.py` соответственно разрешает `flex` и
    для `type == "vacancy"`.
  - **Не тронуто, осталось на будущее** (см. `ANTIGRAVITY_TASK.md`, если
    ещё не удалён): `artists.py` — «Очистить поле» контакта может стереть
    «неудаляемый» базовый контакт; `releases.py` — `_send_screen()` без
    fallback при сбое отправки фото; `user_extra_fields.py` — нет защиты
    от двойного клика на select/checkbox доп.поле; `market_add.py` —
    отправка фото-превью без try/except; `vacancy_view.py` — плашка
    «с учётом опечатки» пропадает при возврате из карточки к результатам
    поиска; ещё несколько `.scalar_one()` без `_or_none()` в
    `market_view.py`/`market_edit_overview.py`. Тесты: 147/147.
  - **Веб-админка (`category_admin.py`), тот же период:** список
    объявлений («📦 Объявления») получил переключатель «🖼 Превью» /
    «📋 Таблица» — таблица с теми же настраиваемыми столбцами, что и у
    «Тексты» (показать/скрыть, drag-переупорядочивание, resize, всё в
    localStorage), плюс сортировка по клику на заголовок (в т.ч. по
    просмотрам — «какой клип/объявление популярнее»). `/api/listings`
    получил `sort`/`order` с whitelist колонок (защита от SQL-инъекции
    через ORDER BY). Заодно унифицирован формат даты/времени по всей
    админке на `ДД.ММ.ГГГГ ЧЧ:ММ` (было — голый срез ISO-строки без года
    и времени в карточках/таблице каталога, при этом в модалке
    объявления уже был нормальный формат — теперь везде одна общая
    функция `fmtDateTime()`, а не три разных её копии).
