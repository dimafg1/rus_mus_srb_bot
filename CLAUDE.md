...
- **Тексты (2026-07-21):** вкладка «📝 Тексты» — редактор `BotText` (поиск,
  ...
  **▶ ПРОДОЛЖИТЬ ОТСЮДА (2026-07-21, конец сессии):** весь согласованный
  ...
  — готов, проверен вживую, закоммичен. Открытых задач и известных багов
  нет. Следующий шаг — по решению владельца, кандидаты без даты:
  1) заполнить пустые `text_en`/`text_kk` в `BotText` (содержательная
     работа с переводами, не техническая — сотни строк, отдельный проход);
  2) партнёрские кампании — UI в веб-админке для таблицы `campaign`,
     дизайн ещё не обсуждали;
  3) переключение языка пользователем в боте (RU/EN/KK) — вся сегодняшняя
     и вчерашняя подготовка (перенос текстов, колонка `text_kk`) именно
     под это, сам переключатель ещё не реализован;
