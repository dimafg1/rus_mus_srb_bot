  ...
  **ничего не меняет для пользователей**, пока не построено переключение
  ...
  2. **Дедуп общих фраз-сообщений** (не кнопок) в `BotText` — одна запись на
     ...
     **Готово:** «Некорректные данные.» (23 места, 6 файлов) →
     `err_invalid_data`, паттерн подключения:
     ```python
     await cb.answer(await get_text("err_invalid_data", "ru") or "Некорректные данные.", show_alert=True)
     ```
     (везде это был `cb.answer(..., show_alert=True)` — toast, не сообщение;
     `get_text` добавлен в импорт из `app.routers.utils` в каждом файле).
     **Осталось из исходного списка кандидатов:**
     «Можно редактировать только свои услуги.» (18, готового кода в БД
     нет — завести новый), «Объявление не найдено.» (16 → готов
     `err_listing_404`), «Можно редактировать только свои объявления.»
