...
- **Тексты (2026-07-21):** вкладка «📝 Тексты» — редактор `BotText` (поиск,
  ...
  **Важный урок:** тесты бота (`tests/`) ходят в РЕАЛЬНУЮ `dev.db`, а не в
  ...
  для текстов, где сейчас пусто (отдельный проход, не сегодня), Feature
  Toggles UI поверх `feature_flags`, затем отдельными заходами — бан на
  запись (нужна миграция схемы `BotUser`, поля пока нет) и рассылки (риск
  спама/лимитов Bot API, отдельный дизайн). Архитектурный рефакторинг
  (Vue/React, REST API, SQLAlchemy вместо сырого `sqlite3`, TWA-авторизация)
  — сознательно отложен до решения о публичном сайте, см.
  `[[category-admin-roadmap]]` в памяти.
