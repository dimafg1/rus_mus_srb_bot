  ...
  **ничего не меняет для пользователей**, пока не построено переключение
  ...
  2. **Дедуп общих фраз-сообщений** (не кнопок) в `BotText` — одна запись на
     ...
     **Осталось из исходного списка кандидатов:**
     ...
     нет — завести новый), «Можно редактировать только свои объявления.»
     (17, не было в исходном списке плана, но топ-3 по частоте → готов
     `err_not_owner`), «Некорректная ссылка.» (12), «Фото не найдено.» (9),
