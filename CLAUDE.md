...
- **Тексты (2026-07-21):** вкладка «📝 Тексты» — редактор `BotText` (поиск,
  ...
  **Настройки (тот же день):** вкладка «⚡ Настройки» — тумблеры
  ...
  отдельными заходами — бан на запись (нужна миграция схемы `BotUser`,
  поля пока нет; решено: бан только на запись/публикацию, просмотр
  разделов остаётся доступен) и рассылки (риск спама/лимитов Bot API,
  отдельный дизайн). Архитектурный рефакторинг (Vue/React, REST API,
