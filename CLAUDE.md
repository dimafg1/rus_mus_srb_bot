...
- **Тексты (2026-07-21):** вкладка «📝 Тексты» — редактор `BotText` (поиск,
  пагинация, модалка с textarea RU/EN, без WYSIWYG — тексты содержат
  Telegram HTML-разметку) + редактор `menu` (плоская таблица, инлайн-поля
  иконка/RU/EN/видимость). API: `/api/texts`, `/api/menu`. Дальше по плану
  (согласовано с владельцем): Feature Toggles UI поверх `feature_flags`,
  затем отдельными заходами — бан на запись (нужна миграция схемы `BotUser`,
  поля пока нет) и рассылки (риск спама/лимитов Bot API, отдельный дизайн).
  Архитектурный рефакторинг (Vue/React, REST API, SQLAlchemy вместо сырого
  `sqlite3`, TWA-авторизация) — сознательно отложен до решения о публичном
  сайте, см. `[[category-admin-roadmap]]` в памяти.
