...
- **Тексты (2026-07-21):** вкладка «📝 Тексты» — редактор `BotText` (поиск,
  ...
  **Найденный баг:** поиск по имени сначала использовал SQL `LIKE`,
  ...
  Дальше по плану (согласовано с владельцем): заполнить `text_en`/`text_kk`
  для текстов, где сейчас пусто (отдельный проход, не сегодня), затем
  рассылки (риск спама/лимитов Bot API, отдельный дизайн — обсудить
  перед реализацией). Архитектурный рефакторинг (Vue/React, REST API,
