...
from app.routers.utils import (
    clear_bot_messages, safe_edit_or_send,
    last_bot_messages, sent_photo_messages,
    render_flex_block, render_contact,
...
def _services_categories_kb(cats, city_id: int, parent_id: int) -> InlineKeyboardMarkup:
    """Клавиатура категорий/подкатегорий услуг."""
    rows = [[InlineKeyboardButton(text=c.name, callback_data=f"sv:cat:{city_id}:{c.id}")] for c in cats]
...
def _services_listings_kb(items, city_id: int, cat_id: int) -> InlineKeyboardMarkup:
    """Клавиатура списка услуг в листовой категории."""
    rows = [[InlineKeyboardButton(text=(i.title or f"#{i.id}")[:64],
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=rows)
    print(f"[services_view.py] _services_listings_kb ✓ | city_id={city_id} cat_id={cat_id} items={len(items)}")
    return kb
...
async def sv_city(cb: CallbackQuery):
    ...
    kb = _services_categories_kb(cats, city_id=city.id, parent_id=SERVICES_ROOT_CATEGORY_ID)
...
async def sv_cat(cb: CallbackQuery):
    ...
    async with SessionLocal() as s:
        ...
        if going_back:
            ...
            if not parent_id or parent_id == SERVICES_ROOT_CATEGORY_ID:
                ...
                kb = _services_categories_kb(top_cats, city_id=city_id, parent_id=SERVICES_ROOT_CATEGORY_ID)
                text = f"🛎 Услуги → <b>{city.name}</b>\nВыберите категорию:"
                msg = await cb.bot.send_message(chat_id, text, reply_markup=kb, parse_mode="HTML")
                last_bot_messages[chat_id] = [msg.message_id]
                await cb.answer()
            ...
            kb = _services_categories_kb(siblings, city_id=city_id, parent_id=parent_id)
            text = (
    ...
    if children:
        kb = _services_categories_kb(children, city_id=city_id, parent_id=cat_id)
    ...
    if not items:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sv:cat:{city_id}:{cat.parent_id or SERVICES_ROOT_CATEGORY_ID}:back")]
        ])
    ...
    kb = _services_listings_kb(items, city_id=city_id, cat_id=cat_id)
    text = f"🛎 Услуги → <b>{city.name}</b>\nКатегория: <b>{cat.name}</b>\nВыберите услугу:"
    msg = await cb.bot.send_message(chat_id, text, reply_markup=kb, parse_mode="HTML")
    last_bot_messages[chat_id] = [msg.message_id]
    await cb.answer()
...
async def sv_item(cb: CallbackQuery):
    ...
    main_block = "\n\n".join([p for p in [title_line, descr_line, price_line] if p])
    ...
    # 👉 Если есть YouTube/URL — добавляем ссылку ПЕРЕД контактами (чтобы карточку можно было переслать со ссылкой)
    if video_url:
...
@router.message(ServiceSearch.waiting_for_query)
async def handle_services_search_query(m: Message, state: FSMContext):
    """
    Поиск по услугам: title/descr, только актуальные записи раздела «Услуги».
    Регистронезависимо для кириллицы (через набор LIKE-вариантов).
    """
    chat_id = m.chat.id
    ...
    # Варианты для регистронезависимого поиска в SQLite (кириллица)
    q       = query
    q_cap   = (q[:1].upper() + q[1:]) if q else q            # «Ведущий»
    q_upper = q.upper()                                       # «ВЕДУЩИЙ»
    ...
    async with SessionLocal() as s:
        stmt = (
            ...
            # только актуальные
            .where(or_(Listing.is_sold == 0, Listing.is_sold == False, Listing.is_sold.is_(False)))  # noqa: E712
            # только «Услуги»: по type='service' (с trim/lower) ИЛИ по ветке категорий от id=80
            .where(
                or_(
            ...
            # регистронезависимо по двум полям — 3 варианта LIKE
            .where(
                or_(
                    Listing.title.like(f"%{q}%"),
                    Listing.title.like(f"%{q_cap}%"),
                    Listing.title.like(f"%{q_upper}%"),
                    Listing.descr.like(f"%{q}%"),
                    Listing.descr.like(f"%{q_cap}%"),
                    Listing.descr.like(f"%{q_upper}%"),
                )
            )
            .order_by(Listing.created_at.desc())
            .limit(50)
    ...
    if not results:
        kb = InlineKeyboardMarkup(inline_keyboard=[[new_search_btn], [back_btn]])
        msg = await m.answer(
    ...
    rows = []
    for row in results:
        title = (row.title or f"#{row.id}")[:64]  # лимит Telegram на длину текста кнопки
        rows.append([InlineKeyboardButton(
            text=title,
            callback_data=f"sv:item:{row.id}:{row.city_id}:{row.category_id}:s"
        )])
    rows.append([new_search_btn])
    ...
    await state.set_state(ServiceSearch.waiting_for_detail)
    print(f"[services_view.py] services_search ✓ | chat_id={chat_id} results={len(results)} query={query!r}")
