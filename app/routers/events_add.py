...
# @router.message(StateFilter(AfishaEditStates.city))
# async def afisha_edit_city_apply(message: Message, state: FSMContext):
#     func = "afisha_edit_city_apply"
#     chat_id = message.chat.id
#     owner_id = message.from_user.id
...
#         _af_dbg(func, "db.ok", f"id={listing_id} city_text={raw!r}")
#     except Exception as e:
#         _af_dbg(func, "db.fail", f"{type(e).__name__}: {e}")
#         await message.answer("Не удалось сохранить город. Попробуйте ещё раз.")
#         return
...
#     await state.update_data(city_text=raw, edit_prompt_msg_id=None)
#     await state.set_state(None)
