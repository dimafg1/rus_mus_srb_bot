...
def _fmt(v):
    ...
    if isinstance(v, list):
        return f"<i>{', '.join(map(str, v))}</i>"
    if isinstance(v, dict):
        return f"<i>{json.dumps(v, ensure_ascii=False)}</i>"
    return f"<i>{str(v)}</i>"
...
def _build_overview_text(listing: Listing, city: City | None, cat: Category | None, defs: list[dict], flex_vals: dict):
    ...
    if city: lines.append(f"Город: {city.name}")
    if cat:  lines.append(f"Категория: {cat.name}")
    lines.append("")
        ...
        if ftype == "video":
            if isinstance(cur, str) and cur.strip():
                ...
                if "http" in low or "://" in cur:
                    # Печатаем сам URL
                    lines.append(f"<b>{label}:</b> <i>{cur}</i>")
                else:
...
async def sef_main_title(cb: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as s:
        l = (await s.execute(select(Listing).where(Listing.id == listing_id))).scalar_one()
...
async def sef_main_value_entered(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as s:
        ...
        if field == "title":       l.title = val
        elif field == "price":     l.price = val
...
async def sefx_video_link(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as s:
        l = (await s.execute(select(Listing).where(Listing.id == l_id))).scalar_one()
        try:
            flex = json.loads(l.flex or "{}")
            if not isinstance(flex, dict):
                flex = {}
...
async def sextra_add_services(cb: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as s:
        ...
        rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sextra:back:{listing_id}")])
    ...
    async with SessionLocal() as s:
        ...
        if children:
            ...
            if cat.parent_id and cat.parent_id != 80:
                rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sextra:up:{listing_id}:{cat.parent_id}")])
            else:
                rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sextra:add:{listing_id}")])
...
async def sextra_up_services(cb: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as s:
        ...
        if parent.parent_id and parent.parent_id != 80:
            rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sextra:up:{listing_id}:{parent.parent_id}")])
        else:
            rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"sextra:add:{listing_id}")])
